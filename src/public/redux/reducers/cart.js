...
const cart = (state = initialState, action) => {
  switch (action.type) {
    ...
    case 'EDIT_CART_FULFILLED':
      const dataAfterEdit = state.cart.map(item => {
        // eslint-disable-next-line
        if (
          item.id == action.payload.data.data.id &&
          item.branch == action.payload.data.data.branch
        ) {
          return {
            ...item,
    ...
    case 'DELETE_CART_FULFILLED':
      ...
      state.cart.map(item => {
        ...
        if (
          !(
            item.itemID == action.payload.data.data.item &&
            item.branchID == action.payload.data.data.branch
          )
        ) {
